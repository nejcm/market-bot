...
For company subjects, Web Gather derives which durable business-profile sections a gathered SEC 10-K/10-Q packet already covers (see [[web-subject-profile]]) and passes that coverage into the model context so Exa budget targets recency, corroboration, or genuine gaps instead of re-gathering filed facts; a background `web_search` that duplicates covered filing sections without such a rationale is rejected as `sec-covered-durable-profile`, visible in the web-gather audit. When a durable Web Subject Profile is reused into the run (`reusedProfileCoverage.present`), fresh searches that leave `numResults` unset default to a narrower per-query ingestion (3 instead of 5) since gather then only backfills recency, corroboration, or gaps; an explicit model-supplied `numResults` is respected.
