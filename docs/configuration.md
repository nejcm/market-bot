...
| `MARKET_BOT_INDEX_DB_PATH` | Derived from `MARKET_BOT_DATA_DIR` | Path for the derived SQLite Run Artifact Index. Defaults to `data/index.sqlite` for `data/runs`; if `MARKET_BOT_DATA_DIR` does not end in `runs`, defaults inside that directory. |
| `MARKET_BOT_INDEX_DISABLE` | `false` | Set to `1` or `true` to force disk-scan artifact readers even when the SQLite index exists. This is a permanent recovery escape hatch. |
