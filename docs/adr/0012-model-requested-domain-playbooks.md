...
Selector failures are trace-only. Malformed JSON, unknown IDs, invalid stages, duplicates, and cap overages are recorded in `trace.domainPlaybooks.rejected`; the research run continues with the valid subset or no playbooks. The selector output is persisted in `stages.json`, and selector token/cost estimates count toward run totals.
