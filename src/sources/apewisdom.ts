...
function parseCandidate(
  ...
  const mentions = readInteger(row.mentions);
  const upvotes = readInteger(row.upvotes);
  if (
    ticker === undefined ||
  ...
  const name = readString(row.name) ?? ticker;
  const rank24hAgo = readPositiveInteger(row.rank_24h_ago);
  const mentions24hAgo = readInteger(row.mentions_24h_ago);
...
function parsePage(payload: unknown, filter: string, pageUrl: string): ApeWisdomPage | SourceGap {
  const page = readObject(payload);
  const results = Array.isArray(page?.results) ? page.results : undefined;
  if (results === undefined) {
    return malformedGap("ApeWisdom response missing results array");
  }
