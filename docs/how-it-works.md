...
Resolved prior **misses** are surfaced as explicit error-correction signal in the prompt — the model is asked to diagnose why a prior view was wrong before restating a similar one. Two non-overlapping blocks: ticker runs get an instrument-scoped block on the command's own symbol; market overview runs get a market-scoped block on their configured index/macro/crypto forecast subjects, drawn only from prior same-horizon, same-asset market updates. Hits are never itemized here — they inform the run only through aggregate calibration.
