...
The Research Subject Registry (`src/research/subject-registry.ts`, [ADR 0027](./adr/0027-subject-proxy-peer-universe-registry.md)) is a checked-in equity-only registry for the `research` run type. It maps aliases to canonical subject keys, representative instruments, provenance, and optional single listed ETF prediction proxies. The current public CLI does not parse `research <subject>` yet, but report validation, historical context, artifact indexing, and search already understand `jobType: "research"`. The registry is deterministic and local; no model or provider call can create a scored thematic proxy in V1.
