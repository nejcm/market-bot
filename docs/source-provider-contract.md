...
- Make `SourceGap`s carry typed provider/capability/cause meaning plus a stable human-readable message. Causes should distinguish missing credential, fetch failure, circuit open, stale cache fallback, in-window reuse, unsupported coverage, repeat fallback, malformed response, validation failure, and provider data missing.
