...
describe("run-artifact-layout", () => {
  ...
  test("every value containing a slash is prefixed with its top-level dir", () => {
    const slashed = FILE_VALUES.filter((path) => path.includes("/"));
    for (const path of slashed) {
      const ok = path.startsWith(`${NORMALIZED_DIR}/`) || path.startsWith(`${RAW_DIR}/`);
      expect(ok).toBe(true);
    }
  });

  test("every normalized/ value is prefixed with NORMALIZED_DIR", () => {
  ...
  test("raw/ sidecars are prefixed with RAW_DIR", () => {
    const raw = FILE_VALUES.filter((path) => path.startsWith(`${RAW_DIR}/`));
    expect(raw.length).toBeGreaterThan(0);
    for (const path of raw) {
      expect(path.startsWith(`${RAW_DIR}/`)).toBe(true);
    }
  });
