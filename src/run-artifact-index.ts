  ...
  const outcomes = await Promise.all(
    entries
      ...
      .map(async (entry) => {
        ...
        return await readRunSubsystemOutcomesFromDisk(runDir, report?.runId ?? entry.name);
      }),
  );
  return outcomes
    .flat()
    .toSorted((left, right) => Number(left.runId > right.runId) - Number(left.runId < right.runId));
}
