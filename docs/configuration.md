...
| `MARKET_BOT_MASSIVE_API_KEY` | — | Enables Massive, formerly Polygon.io, as a supplemental equity source provider using `api.massive.com`. Missing key silently disables Massive. When set, failed Massive requests emit `SourceGap`s. Massive contributes equity news and supplemental equity market snapshots only; it does not affect mover ranking, market regime labels, crypto, or scoring Observations. |
