...
Massive, formerly Polygon.io, is supplemental-only. When configured, it uses `api.massive.com` to collect equity news and stock snapshots for the symbols already selected by Yahoo. Those snapshots are persisted as supplemental market snapshots, included as report Sources, and included in prompt evidence. Configured Massive failures remain visible as `SourceGap`s with `evidenceQualityImpact: "no-cap"` because core Yahoo/FRED/CoinGecko semantics decide evidence quality. Massive does not enter mover ranking, market regime summaries, crypto workflows, or scoring Observations.
...
Equity ticker runs also collect live Ticker Regime Context from the regime proxy set so ticker Research Views can cite current breadth and volatility context without substituting prior Market Update artifacts for live evidence.
