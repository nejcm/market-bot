...
- **Market-scoped forecast correction** — for daily/weekly runs, prior misses on the
  run's *configured market subjects* (the fixed index/macro/crypto subjects from
...
- **Extend the instrument block to market-update spotlights — rejected.**
  ...
  spotlights AAPL never emits a scored Prediction *about* AAPL, so there is no
...
- **Add a separate market-scoped forecast correction — accepted.** The same
  same-subject-forthcoming-prediction logic that justifies the ticker block *does*
...
- Market-update runs now carry their own error-correction channel
  ...
  "loaded but unused" caveat applies only to *spotlight ticker* history, not to the
