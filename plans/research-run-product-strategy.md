...
| **Subject playbooks ("skills for research runs")** — *shipped* | Per-subject Domain Playbooks deterministically seated when the run's resolved `subjectKey` matches a registry `subjectKeys` filter (e.g. `subject-biotech`), layering domain-specific analytical discipline on top of the mandatory `thematic-research` frame. | Raises research-run quality with domain-specific framing (regulatory cadence, M&A discipline, proxy caveats) without loosening the research-only boundary or ranked-pick prohibition. | Low — reuses the existing playbook registry, eligibility, and mandatory-seating machinery. | `prompts/playbooks/` + `src/research/playbooks.ts` | codex `public-equity-investing` skill mechanism |
