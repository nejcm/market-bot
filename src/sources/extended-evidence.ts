...
async function collectFred(ctx: CollectContext): Promise<ProviderResult> {
  ...
  for (const [index, result] of results.entries()) {
    if (isFetchJsonResult(result)) {
      const value = latestNumber(readArray(result.payload, "observations"), ["value"]);
      if (value !== undefined) {
...
async function collectExtendedEvidence(
  ...
  const providerResults = await Promise.all([
    collectSec(ctx),
    collectFinnhubEvents(ctx),
    collectFred(ctx),
    collectTradierIv(ctx),
    collectGlassnode(ctx),
  ]);
