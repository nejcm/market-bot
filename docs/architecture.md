...
- `src/scoring/calibration.ts` + `calibration-markdown.ts` + `calibration-console.ts` — aggregate scored predictions sliced by cadence (daily / weekly / ticker) into `data/calibration/`, including Miss Autopsy taxonomy counts; the `calibration` CLI prints a stdout reliability dashboard
