...
`bun run check` is the local definition of done: format, lint, format check, TypeScript/Svelte checks, app build, and tests.
...
Instrument jobs include recent same-symbol instrument runs plus same-asset market overview history, including readable legacy daily/weekly artifacts. Deep-equity acquisition is recipe-driven and deterministic; historical context does not direct provider requests.
...
- Eligible deep equity runs merge deterministic SEC and Tradier packet output before model analysis.
